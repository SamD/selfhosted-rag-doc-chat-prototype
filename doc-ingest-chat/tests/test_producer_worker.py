...
def test_is_bad_ocr_true():
    with patch("utils.producer_utils.get_tokenizer") as mock_get:
        ...
        with patch("utils.producer_utils.is_gibberish", return_value=True), patch("utils.producer_utils.is_visibly_corrupt", return_value=False), patch("utils.producer_utils.is_low_quality", return_value=False):
...
def test_is_bad_ocr_false():
    with patch("utils.producer_utils.get_tokenizer") as mock_get:
        mock_tokenizer = MagicMock()
        mock_get.return_value = mock_tokenizer
        with patch("utils.producer_utils.is_gibberish", return_value=False), patch("utils.producer_utils.is_visibly_corrupt", return_value=False), patch("utils.producer_utils.is_low_quality", return_value=False):
