...
class IngestState(TypedDict):
    ...
    status: str
    error: Optional[str]
    doc_preview: Optional[str]   # Sample text for the supervisor
    doc_context: Optional[str]   # 1-sentence persona generated by LLM
    ...
    try:
        if file_type == "pdf":
            with pdfplumber.open(full_path) as pdf:
                # Take first 2 pages for context
        ...
        # Limit preview to 2000 chars to keep supervisor latency low
        return {**state, "doc_preview": (preview_text or "")[:2000]}
    except Exception as e:
        log.warning(f"⚠️ Preview node failed for {state['rel_path']}: {e}")
        return state
...
def supervisor_node(state: IngestState) -> IngestState:
    ...
    try:
        ...
        # Prompt engineered for Qwen2.5-Instruct metadata extraction
        prompt = (
            "<|im_start|>system\nYou are a precise metadata extractor. Your task is to describe the document provided in a single, concise sentence. "
            "Focus on the main subject, author, and date. Output ONLY the sentence.<|im_end|>\n"
            f"<|im_start|>user\nExtract context from this text sample:\n\n{preview}<|im_end|>\n"
            "<|im_start|>assistant\n"
        )
...
def stream_chunks_to_redis(chunks_with_engine, state: IngestState) -> int:
    """Helper to format and push chunks to Redis."""
...
def pdf_extract_node(state: IngestState) -> IngestState:
    """Parallel extraction with Context injection."""
    full_path = state["full_path"]
    rel_path = state["rel_path"]
...
def html_extract_node(state: IngestState) -> IngestState:
    """HTML extraction with Context injection."""
    full_path = state["full_path"]
    rel_path = state["rel_path"]
    doc_context = state["doc_context"]
...
def media_extract_node(state: IngestState) -> IngestState:
    ...
    try:
        ...
        # Note: doc_context could be injected here too if desired
        chunks, _ = split_doc(text, rel_path, "media", page_num=-1, doc_context=state["doc_context"])
...
def fallback_ocr_node(state: IngestState) -> IngestState:
    """OCR fallback with Context injection."""
    full_path = state["full_path"]
    rel_path = state["rel_path"]
    doc_context = state["doc_context"]
...
def create_producer_graph():
    """Builds the Producer StateGraph with Supervisor logic."""
    ...
    # 1. After Scan -> Get Preview
    workflow.add_edge("scan_file_node", "preview_node")
    
    # 2. After Preview -> Run Supervisor
    workflow.add_edge("preview_node", "supervisor_node")
    
    # 3. After Supervisor -> Route to proper extraction
    def route_after_supervisor(s):
