# Ingestion Lifecycle Recovery

## Purpose

The ingestion lifecycle recovery capability provides automated and manual mechanisms for recovering from worker crashes and partial ingestion failures. It handles cleanup of orphaned state (Redis queue entries, DuckDB staged chunks) when jobs fail, supports re-ingestion of failed files, and provides a manual reclaim method for jobs stuck in intermediate states.
...
The system SHALL expose a STUCK_JOB_TIMEOUT_HOURS environment variable (default: 1) that controls the threshold for considering a job orphaned. Jobs that have been in an intermediate state for longer than this threshold SHALL be eligible for reclaim.
