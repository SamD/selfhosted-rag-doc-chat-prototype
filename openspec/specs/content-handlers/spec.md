...
The content handlers capability implements a Chain of Responsibility pattern for extracting raw text from multiple file formats. Each handler specializes in a specific MIME type (PDF, video, audio, text) and can delegate to downstream workers (OCR for scanned PDFs, WhisperX for media files). The MIME type determined by the handler flows through Redis job metadata to ensure downstream workers send the correct Content-Type header.
