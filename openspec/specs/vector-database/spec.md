...
The vector database capability provides a unified abstraction over Qdrant and Chroma for storing and retrieving document embeddings. It manages embedding generation via the e5-large-v2 model (local or remote), handles consumer chunk staging in DuckDB, and archives processed data to Parquet files for long-term storage.
