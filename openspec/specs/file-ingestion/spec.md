...
The file ingestion capability manages the end-to-end lifecycle of document processing, from discovery in the staging directory through to final storage in the vector database and archival to Parquet. It implements a DuckDB-backed state machine with atomic transitions and coordinates 4 workers (Gatekeeper, Producer, Consumer x2) via Redis queues.
