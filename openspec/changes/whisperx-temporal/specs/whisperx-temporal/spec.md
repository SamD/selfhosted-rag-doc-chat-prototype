...
### Requirement: Silent loss on worker crash (WhisperX only)

The WhisperX worker SHALL NOT silently lose in-progress transcription jobs when the worker container crashes. This is a behavioral removal specific to the WhisperX path — other workers retain their existing silent-loss behavior until they are also migrated.

**Reason**: Temporal Activity durability replaces the previous "push to Redis, pray the worker stays alive" model.

**Migration**: Files whose transcription was lost due to a crash will be detected by the staging directory audit (existing mechanism) and re-queued, but under normal operation no files will be lost.
